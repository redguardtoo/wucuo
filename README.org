...
Please note =wucuo= is a complete solution. You should not turn on =flyspell-prog-mode= or =flyspell-mode= when using wucuo.

See =wucuo-check-nil-font-face= on how to check plain text (text without font).
* Tips
** Change start mode
The default value of =wucuo-flyspell-start-mode= is "ultra".
